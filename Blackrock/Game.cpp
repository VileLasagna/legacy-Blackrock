...
void Game::Run()
{
	...
	{
		SDLDisplay::instance()->Update(); //Get the time and all before everyone starts checking this kind of stuff
		i = Update();
		Draw();
		Flip();
		currentState = i;
...
void Game::drawFPS()
{
	...
	static std::string u = "";
    frames++;
    t += SDLDisplay::instance()->getDtSecs(); // update time..
...
void Game::addObject(std::string type, GameObject *object)
{
	std::pair<std::string,std::pair<GameObject*,ObjStatus>> ob(type,std::pair<GameObject*,ObjStatus>(object,ObjStatus()));
	Objects.insert(ob);
}
